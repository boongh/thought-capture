...
The embedding model runs locally rather than through OpenRouter. Text embedding is inexpensive to host, and keeping it local removes per-run cost, removes a network round trip from the organize path, and avoids disclosing document content to a second provider (section 12.1). At Release 1 corpus size these vectors are stored as `float4[]` alongside the revision and compared by direct cosine similarity; pgvector is unnecessary for *this* signal until a linear scan over it is measurably expensive. The pinned `postgres` image does now carry the extension (`deploy/compose/postgres/Dockerfile`, `docs/adr/0010`), so staying on `float4[]` here is a sizing choice, not a missing capability - section 8's document semantic index is the pgvector-backed store. Changing the embedding model requires a `reembed` run and is recorded in `runs.embedding_model_id`.
