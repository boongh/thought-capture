...
observed against a live configured instance (see that amendment's own
"Context" for why that differs from this ADR's ``/api/search``/``/api/content``
spike). The unconfigured-instance failure path *was* live-verified (a plain
HTTP 500) and is exercised by the "unreachable" test below via the same
mechanism.
"""
