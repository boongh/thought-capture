...
- Use `.claude/agents/integration-reviewer.md` for the failures a per-file reader structurally cannot see - contract drift between caller and callee, acknowledgement ordered before durable commit, workspace scoping lost at a handoff, schema/migration divergence, configuration declared but never read, a new suite neither check script runs. It reads unchanged files on purpose and, like `code-reviewer`, has no shell access and edits nothing.
- Use `.claude/agents/fix-implementer.md` only to implement an already-triaged, already-approved fix inside an explicitly assigned set of files. It is not a reviewer or a planner, it never touches git state, Docker, the check scripts, or the lockfile, and it must stop and report rather than widen its own scope.
- Use `.claude/skills/first-line-review/SKILL.md` to run those agents as a loop - parallel shard reviews plus one integration review, central triage into an evidenced fix plan, parallel fixes with disjoint file ownership, then one serial verification run and a re-review of the fixes. Parallel fix agents share a single checkout, so the skill's file-ownership and exclusive-resource rules are load-bearing, not stylistic.
- Keep final synthesis and architectural decisions in the primary Claude session. Triage is part of that synthesis: a subagent's finding becomes a fix only after the primary session reads the underlying code and agrees the bug is real.
- Do not use these agents as a substitute for asking the owner about consequential project-specific preferences, or for Codex's required independent evaluation where CLAUDE.md's Implementation behavior section calls for it.
