...
# (docs/adr/0003 finding 4, docs/adr/0010); generous relative to the search/
# content endpoints, which never call out to a model at all.
...
class HttpKhojClient:
    ...
    async def chat(self, question: str, *, limit: int = 5) -> KhojChatResult:
        ...
        # in khoj's own source at the exact pinned tag (docs/adr/0010) -
        # `stream=False` returns a plain JSON body instead of an SSE stream,
        # which this synchronous adapter needs. `create_new=True` starts a
        # fresh server-side conversation on every call: this project does not
        # track `conversation_id` on its own side (docs/adr/0010 scopes Ask to
        # single-turn Q&A for this slice), so reusing Khoj's own anonymous-mode
        # default conversation would silently accumulate unrelated history
        # across unrelated questions instead.
        payload = {
        ...
        if not isinstance(answer, str):
            ...
            # model actually returns (docs/adr/0003 finding 4) was not
            # observed against a live instance, so this is deliberately
