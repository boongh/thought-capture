...
class Settings(BaseSettings):
    ...
    # -- Khoj (docs/adr/0003, docs/adr/0010) --------------------------------
    ...
    # Deliberately off by default (docs/adr/0010): resolves docs/adr/0003
    # finding 4's deferred privacy decision the same way docs/adr/0006 gates
    # organize/select on an empty model slug - an explicit, greppable opt-in
    # rather than a silent default. Turning this on sends the question text
    # and Khoj's retrieved note content to whatever chat model Khoj itself is
    # configured with (its own OPENAI_BASE_URL/OPENAI_API_KEY in
    # deploy/compose/khoj.docker-compose.yml, entirely the operator's choice
    # and never set by this project's own code) - outside this project's own
    # OpenRouter adapter and docs/adr/0006's safe/custom mode controls
    # entirely. Two independent opt-ins (this flag, and Khoj's own chat-model
    # configuration) are required before any Ask call can reach a live model.
